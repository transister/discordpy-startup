...
async def showTL():
    ...
    try:
        ...
        tl.reverse()
        for status in tl:
            tweet_data[status.author.id] = {'text':status.text}
            tweet_data[status.author.id] = {'icon_url':status.author.profile_image_url}
            tweet_data[status.author.id] = {'name':status.author.name}
            main_content = {    
                    "username": tweet_data[status.author.id]['name'],
                    "avatar_url": tweet_data[status.author.id]['icon_url'],
                    "content": tweet_data[status.author.id]['text']
            }
